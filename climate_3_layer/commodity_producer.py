...
class CommodityProducer(abce.Agent, abce.Firm):
    ...
    def production(self):
        ...
        for input_good, required_quantity in self.inputs.items():
            ...
            print(f"    Commodity Producer {self.id}: Has {available:.2f} {input_good} (needs {required_quantity:.2f} per unit)")
        ...
        if max_production_with_climate > 0:
            ...
            print(f"    Commodity Producer {self.id}: No production possible - insufficient inputs")
