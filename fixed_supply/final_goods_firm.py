...
class FinalGoodsFirm(abce.Agent, abce.Firm):
    def init(self, config):
        """ Final goods firms in the fixed supply model maintain constant output.
        They buy intermediate goods and labor, adjusting purchases based on 
        climate-affected productivity, and calculate prices dynamically.
        ...
        self.household_count = config['household_count']
        
        print(f"Fixed Supply Final Goods Firm {self.id} initialized:")
    ...
    def buy_inputs_optimally(self):
        """ Buy intermediate goods and labor to maintain fixed production """
        required_inputs = self.calculate_required_inputs()
        
        print(f"    Fixed Supply Final Goods Firm {self.id}:")
    ...
    def calculate_dynamic_price(self):
        ...
        if self.production_this_round > 0:
            ...
            if self.climate_productivity < 1.0:
                ...
                # BUT: upstream price increases are automatically included in cost_per_unit
                target_price = cost_per_unit * (1 + self.profit_margin)
                self.climate_cost_burden = 0
                
                print(f"    Dynamic pricing for Final Goods Firm {self.id}:")
                print(f"      Cost per unit: ${cost_per_unit:.2f} (includes upstream price effects)")
                print(f"      No own climate impact")
    ...
    def calculate_sales_after_market_clearing(self):
        ...
        if hasattr(self, 'inventory_before_sales'):
            ...
            print(f"    Financial summary for Final Goods Firm {self.id}:")
    ...
    def repay_debt(self):
        ...
        if self.debt > 0 and self['money'] > 0:
            ...
            if debt_payment > 0:
                ...
                print(f"    💰 Final Goods Firm {self.id}: Repaid ${debt_payment:.2f} debt")
    ...
    def apply_climate_stress(self, stress_factor):
        ...
        print(f"  Final Goods Firm {self.id}: CLIMATE STRESS! Productivity: {self.climate_productivity:.2f}")
    ...
    def reset_climate_stress(self):
        ...
        if self.climate_stressed:
            ...
            print(f"  Final Goods Firm {self.id}: Climate stress cleared, productivity: {self.climate_productivity:.2f}")

    def apply_acute_stress(self):
        """ Apply acute climate stress """
        stress_factor = 1.0 - (self.climate_vulnerability * random.uniform(0.2, 0.8))
        self.climate_productivity = stress_factor * self.chronic_stress_accumulated
        print(f"  Final Goods Firm {self.id}: Acute stress! Productivity: {self.climate_productivity:.2f}")
